...
async function run() {
  ...
  try {
    ...
    reporter.expect(
      'ASIN 0374157359 (Dawn of Everything) extracted',
      asins.includes('0374157359'),
    );
    reporter.expect(
      'ASIN 0262548712 (Moral Codes) extracted',
      asins.includes('0262548712'),
    );
    ...
    await offersPage.addInitScript(() => {
      ...
      window.chrome = {
        runtime: {
          id: 'test-harness',
          onMessage: {
            addListener: (cb) => { window.__offersListener = cb; },
          },
        },
      };
    });
    ...
    const listenerPresent = await offersPage.evaluate(() => typeof window.__offersListener === 'function');
    reporter.expect('offers-content.js registered listener', listenerPresent);

    const response = await offersPage.evaluate(() => new Promise((resolve) => {
    ...
    reporter.expect('extractOffers responded', response && response.success === true,
    ...
    reporter.expect('lowest offer has a positive price', !!lowest && lowest.price > 0,
    ...
    const parityResponse = await parityPage.evaluate(() => new Promise((resolve) => {
      window.__offersListener(
        { action: 'extractOffers', productType: 'book', currentFormat: null },
        null,
        resolve,
    ...
    const parityOffers = (parityResponse && parityResponse.offers) || [];
    ...
    reporter.expect('parity extractOffers responded', parityResponse && parityResponse.success === true,
      parityResponse && parityResponse.error);
    reporter.expect(
    ...
    const amazonAt1999 = parityOffers.filter(o =>
      o.seller === 'Amazon.com' && Math.abs(o.price - 19.99) < 0.01
    );
    reporter.expect(
    ...
    const sameTotalDifferentSellers = parityOffers.filter(o =>
      Math.abs(o.price - 13.51) < 0.01
    );
    reporter.expect(
    ...
    const kindleLeaked = parityOffers.some(o =>
      o.seller && o.seller.toLowerCase().includes('amazon digital services')
    );
    reporter.expect('Kindle digital offer filtered out', !kindleLeaked);

    const unknownShipping = parityOffers.find(o =>
      Math.abs(o.price - 22.00) < 0.01 && o.seller === 'BookBarn'
    );
    reporter.expect(
