...
(() => {
  ...
  async function waitForOffersToLoad() {
    ...
    return new Promise((resolve) => {
      ...
      const checkDiv = () => {
        ...
        const div =
          document.getElementById("all-offers-display") ||
        ...
        const noSellers = div.textContent
          .toLowerCase()
  ...
  const SUBCOMPONENT_ID =
    /^aod-(pinned-)?offer-(heading|soldBy|shipsFrom|price|seller-rating|added-to-cart|updated-cart|not-added-to-cart|view-cart|promotion|qty-|upsell|list|main-content-show-more|main-content-show-less|additional-content|show-more-link|show-less-link)/;
  ...
  function parseCondition(container) {
    const heading = container.querySelector('[id="aod-offer-heading"], [id^="aod-offer-heading-"]');
    ...
    const raw = heading.innerText?.trim() ? heading.innerText : heading.textContent;
    return normalizeCondition(raw);
  }

  function parseSeller(container) {
    const sold = container.querySelector('[id="aod-offer-soldBy"], [id^="aod-offer-soldBy-"]');
    if (sold) {
  ...
  function parseOffersFromDiv(root) {
    ...
    if (DEBUG_MODE) {
      ...
      if (unique.length > 0) {
        const best = unique[0];
        console.log(
          `Lowest total: $${best.totalPrice.toFixed(2)} (${best.condition}, ${best.seller})`,
        );
      }
    }
