...
function extractASIN(item) {
  ...
  if (dataAsin && dataAsin.match(/^[A-Z0-9]{10}$/)) {
    return dataAsin;
  }
...
async function fetchAllPrices(asin, title, productType = 'unknown', currentFormat = null) {
  try {
...
function displayPriceInfo(item, prices, isLoading = false, asin = null, productType = 'unknown', currentFormat = null) {
...
function analyzeWishlistItem(item) {
  ...
  const isDigitalBook = formatText.includes('kindle') ||
                        formatText.includes('audible') ||
                        formatText.includes('audiobook') ||
                        formatText.includes('ebook');

  const hasBookFormat = formatText.includes('hardcover') || formatText.includes('hard cover') ||
                        formatText.includes('hardback') || formatText.includes('paperback') ||
                        formatText.includes('paper back') || formatText.includes('softcover') ||
                        formatText.includes('mass market') || lowerTitle.includes('book');
  ...
  if (formatText.includes('hardcover') || formatText.includes('hard cover') || formatText.includes('hardback')) {
    currentFormat = 'hardcover';
  } else if (formatText.includes('paperback') || formatText.includes('paper back') || formatText.includes('softcover')) {
    ...
    const hasNewItems = mutations.some(mutation => {
      return Array.from(mutation.addedNodes).some(node => {
        return node.nodeType === 1 && (
          node.matches && node.matches('[data-itemid]') ||
          node.querySelector && node.querySelector('[data-itemid]')
        );
      });
    });
